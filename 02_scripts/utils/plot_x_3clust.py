...
stlnmfV = np.array(pd.read_csv("DEEPNMF_real3.csv", sep=",", header=None))
# stlnmfV = np.array(pd.read_csv("V_group_sparsity.csv", sep=",", header=None))
stlnmfV /= sum(stlnmfV)
cardV = np.array(pd.read_csv("./CARD/card_real_3.csv", sep=",", header=0)).T
stereoV = np.array(pd.read_csv("stereoscope/pdac_a_stereo/03_X_real/XT_real/W.2024-08-21175235.111338.tsv", sep="\t", header=0, index_col=0))[:,0:3].T
stereoV /= sum(stereoV)

...
def get_data(batch=64):
    ...
    for x_file in X_names: 
        ...
        # X = pd.read_csv("./00_synthetic/00_PDAC_A/02_X_shuf/X_0.tsv", sep="\t").set_index("Genes")
        # # X = pd.read_csv("./00_synthetic/00_PDAC_A/02_X_shuf/"+x_file, sep="\t").set_index("Genes")
        # row_names = pd.read_csv("./Genes.csv", sep=" ", header=0).set_index("Genes")

        # X = X[X.index.isin(row_names.index)]
        # X = np.array(X.sort_index())
        # X = torch.tensor(X, dtype=torch.float32, device=device)

        # # V = pd.read_csv("../CARD/x_shuf/" + V_name, sep=",", header=0)
        # V = pd.read_csv("../CARD/x_shuf/card_X_0.tsv", sep=",", header=0)
        # # V = pd.read_csv("../CARD/card_real.csv", sep=",", header=0)
        # V = torch.tensor(np.array(V), dtype=torch.float32, device=device).T

        # I = pd.read_csv("./00_synthetic/00_PDAC_A/01_X_Noise/05_clust/02_v/intensity/i0_1.csv", sep=" ", header=None)
...
I = pd.read_csv("./00_synthetic/00_PDAC_A/00_No_X_Noise/05_clust/02_v/intensity/i0_1.csv", sep=" ", header=None)
I = torch.tensor(np.array(I), dtype=torch.float32, device=device)
...
def train():
    ...
    for i in range(51):
        ...
        for idx in range(len(inputs)):
            ...
            X, real_V, I, t = inputs[idx][0], inputs[idx][1], inputs[idx][2], inputs[idx][3]
            types.append(t)

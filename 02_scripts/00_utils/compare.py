...
stereoV_Ds = next(os.walk('./stereoscope/pdac_a_stereo/01_X_Noise/5ct'))[1]

stereoVs = [ [0]*2 for i in range(len(stereoV_Ds))]
for idx, D in enumerate(stereoV_Ds):
    v = os.listdir("./stereoscope/pdac_a_stereo/01_X_Noise/5ct/"+D)
    stereoVs[idx][1] = v[0]
    stereoVs[idx][0] = D
...
for pair in stereoVs:
    ...
    real = np.array(pd.read_csv("./00_synthetic/00_PDAC_A/01_X_Noise/05_clust/02_v/v/v"+name[1:-6]+".csv", sep=" ", header=None))
    real /= sum(real)
    stereo = (np.array(pd.read_csv("./stereoscope/pdac_a_stereo/01_X_Noise/5ct/"+pair[0]+"/"+pair[1], sep="\t", header=0, index_col=0)))
    # stereo = np.c_[stereo[:,0],stereo[:,1],stereo[:,2]].T
    stereo = np.c_[stereo[:,0],stereo[:,1],stereo[:,7],stereo[:,9],stereo[:,6]].T
    stereo /= sum(stereo)

    spot = np.array(pd.read_csv("./spotlight/noisy5/spotlight_"+str(name)+".tsv", sep="\t", header=0))
    spot = np.c_[spot[:,0],spot[:,1],spot[:,7],spot[:,9],spot[:,6]].T
    spot /= sum(spot)

    card = np.array(pd.read_csv("./CARD/noisy_5_v/card_"+str(name)+".csv", sep=",", header=0)).T
    card /= sum(card)
    stlnmf = np.array(pd.read_csv("./03_results/noisy/5clust/DEEPNMF_"+str(name)+".csv", sep=",", header=None))
    stlnmf /= sum(stlnmf)
...
ax1.set_title("Comparison of Methods: Noisy, 5 Cell-types", loc="left",
          fontstyle='italic', fontsize=15, weight="bold")

fig.savefig("compare_noisy5.png")
